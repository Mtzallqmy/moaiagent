...
class RuntimeCoreTest {
    @Test fun classifierSeparatesSafeModifyDestructiveAndExternal() {
        ...
        assertEquals(RiskLevel.SAFE, classifier.classify("git branch --list 'phase*'", cwd).risk)
        assertEquals(RiskLevel.MODIFY, classifier.classify("git branch phase3", cwd).risk)
        assertEquals(RiskLevel.DESTRUCTIVE, classifier.classify("git branch -D old", cwd).risk)
        assertEquals(RiskLevel.MODIFY, classifier.classify("git diff --output=changes.patch", cwd).risk)
        assertEquals(RiskLevel.MODIFY, classifier.classify("git show --output snapshot.txt HEAD", cwd).risk)
    ...
    @Test fun policyBlocksSymlinkEscapesAndExplicitRecursiveFollowing() {
        val root = Files.createTempDirectory("runtime-workspace-links").toFile()
        ...
        assertFalse(policy.assess("cat outside-link", root).allowed)
        assertFalse(policy.assess("find -L . -type f", root).allowed)
        assertFalse(policy.assess("grep -R secret .", root).allowed)
        assertFalse(policy.assess("ls -RL .", root).allowed)
    }
    ...
    @Test fun commandPermissionPatternsAreBoundToEffectiveRisk() = runBlocking {
        val root = Files.createTempDirectory("runtime-permission-risk").toFile()
        val manager = ProcessManager(DefaultProcessRunner(), limits = RuntimeLimits())
        val services = object : RuntimeServices {
        ...
        val tool = createRuntimeTools(services).single { it.definition.name == "run_command" }
        val context = ToolContext("ws", "conversation", "session", com.agentdroid.core.agent.AgentMode.AGENT)
        val createKey = tool.permissionKey(buildJsonObject { put("command", "git branch phase3") }, context)
        val deleteKey = tool.permissionKey(buildJsonObject { put("command", "git branch -D old") }, context)

        assertTrue(createKey!!.contains(":MODIFY:"))
        assertTrue(deleteKey!!.contains(":DESTRUCTIVE:"))
        assertNotEquals(createKey, deleteKey)
    }
